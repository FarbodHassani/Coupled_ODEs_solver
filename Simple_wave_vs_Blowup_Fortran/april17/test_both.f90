program test_fcn
  !! gfortran -O6 -ffree-line-length-0 test_both.f90 fcn_both.f90 dopri5_no_rpar.f90 -o both
  ...
  if(numarg.ne.7)then
     ...
     print *,"7: plot shape=0, extrema=1"
     stop
  endif
  A =myarg(1)
  B=myarg(2)
...
!  write(0,*)'it',it
  ...
  uxx=0
  do i=2,n-1
     ux(i)=(current(i+1)-current(i-1))/(2*3.1415/n/2)
     uxx(i)=(-current(i+1)+2*current(i)-current(i-1))/(2*3.1415/n/2)**2
  enddo
  do i=1,n
     current(i)=ux(i)
     current(i+n)=uxx(i)
