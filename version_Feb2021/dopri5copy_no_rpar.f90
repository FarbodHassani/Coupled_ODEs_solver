      subroutine dopri5copy(n,fcn,x,y,xend,                                 &
     &     rtol,atol,itol,                                              &
     &     solout,iout,                                                 &
...
! -------- call to core integrator ------------                         
      call dopcorcopy(n,fcn,x,y,xend,hmax,h,rtol,atol,itol,iprint,          &
...
!                                                                       
      subroutine dopcorcopy(n,fcn,x,y,xend,hmax,h,rtol,atol,itol,iprint,    &
     &     solout,iout,idid,nmax,uround,meth,nstiff,safe,beta,fac1,fac2,&
...
! ----------------------------------------------------------            
      ...
      logical reject,last 
      external fcn 
      common /condo5copy/xold,hout
! *** *** *** *** *** *** ***                                           
!  initialisations                                                      
! *** *** *** *** *** *** ***                                           
      if (meth.eq.1) call cdopricopy(c2,c3,c4,c5,e1,e3,e4,e5,e6,e7,         &
...
! --- initial preparations                                              
      ...
      hmax=abs(hmax) 
      iord=5 
      if (h.eq.0.d0) h=hinitcopy(n,fcn,x,y,xend,posneg,k1,k2,k3,iord,       &
     &     hmax,atol,rtol,itol)                               
      nfcn=nfcn+2 
      reject=.false. 
      xold=x 
...
! --- fail exit                                                         
  ...
  979 format(' exit of dopri5 at x=',e18.4) 
      ...
      END                                           
!                                                                       
      function hinitcopy(n,fcn,x,y,xend,posneg,f0,f1,y1,iord,               &
     &     hmax,atol,rtol,itol)                               
! ----------------------------------------------------------            
...
! ----  h**iord * max ( norm (f0), norm (der2)) = 0.01                  
            ...
            hinitcopy=sign(h,posneg) 
            return 
      END                                           
!                                                                       
      function contd5copy(ii,x,con,icomp,nd) 
! ----------------------------------------------------------            
...
5        continue
      ...
      theta=(x-xold)/h 
      theta1=1.d0-theta
      contd5copy=con(i)+theta*(con(nd+i)+theta1*(con(2*nd+i)+theta*         &
           &     (con(3*nd+i)+theta1*con(4*nd+i))))
      return 
      END                                           
!                                                                       
      subroutine cdopricopy(c2,c3,c4,c5,e1,e3,e4,e5,e6,e7,                  &
     &                    a21,a31,a32,a41,a42,a43,a51,a52,a53,a54,      &
     &                    a61,a62,a63,a64,a65,a71,a73,a74,a75,a76,      &
     &                    d1,d3,d4,d5,d6,d7)                            
! ----------------------------------------------------------            
