program  cosmology
  !!gfortran -O6 -ffree-line-length-0 oneorbit0203.f90 dopri5_no_rpar.f90 dopri5copy_no_rpar.f90
  !! ifort -xHost -O3 -w -qopt-report -prof-use -ipo oneorbit0203.f90 dopri5_no_rpar.f90 dopri5copy_no_rpar.f90  -o ifort_norpar
  ...
  do paramw=0,nw   ! loop over w
     ...
     do paramc=0,nc   !loop over c_s^2
        ...
        if(idid==-15)then
           write(0,*)'singularity detected at taustar'
        endif
        if(idid==-3)then
           write(0,*)'stepsize too small'
        endif
        if(idid==-17)then
           write(0,*)'l2norm > 10^{20}'
        endif
